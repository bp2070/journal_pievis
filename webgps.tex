...
While looking for information in information hierarchies, users can quickly become lost.
Hence, it is important that one be able to categorize, understand, and present information hierarchies efficiently to the users.
...
In this paper we present a novel method to interactively visualize information hierarchies in real-time.
We use the World Wide Web as an application example of our techniques.
...
%Consequently, repeatedly reported problems in WWW navigation are not knowing where you are, not knowing how to get back to previously visited information, and not knowing which sites have already been visited~\cite{eh-89, wls05}.
...
%It is widely agreed that navigation aids in form of maps can visualize the topological structure of the navigational space and can provide a thematic overview, thus supporting users' navigation task and reducing disorientation~\cite{cr-96,jh-02}.
%A regular Web browser's back and forward functionality is a not a sufficient solution to this problem.
%Effective Web visualizations, especially for navigation and search, must cope with such problems, automatically, and if they are to be used as modes of navigation and query, need to be interactive, so that territory and map become one~\cite{atlas}.
%A map (visualization) reduces the user's cognitive load because it abates the load on human long term and
%working memory, summarizing the information about the structure and organization that would otherwise have to be remembered~\cite{a-97, cr-96,jh-02, hsc-97, w-91}.
...
%In Section~\ref{se:vizEng}, we present the engine of our visualization.
%We describe our techniques for interactively exploring our visualization in Section~\ref{se:animation}.
%We detail our labeling method in Section~\ref{se:labels}.
We discuss the real-time and synchronization features in Section~{\ref{se:real-time} and~\ref{se:synch}, respectively.
%We describe how we implemented our system in Section~\ref{se:implementation}.
...
%As mentioned in Section~\ref{se:intro}, in this paper we propose a novel solution to the lost in cyberspace problem.
%Regular Internet users get lost in cyberspace because they have no idea what to expect (no context).
%In addition, there does not exist a consistent, standard methodology for organizing distinct Websites and the users of the Web suffer from information overload because they have the option of too many Web pages to visit~\cite{a-97}.
%A ``lost in cyberspace'' scenario could be as simple as the following:  a user, while browsing the Web, proceeds from {\em Website A} (See Figure~\ref{fig_generic}(a)) when finding an interesting link to {\em Website B} (See Figure~\ref{fig_generic}(b)).
%While at {\em Website B}, the user finds another another interesting link.
%This takes the user to {\em Website F} (See Figure~\ref{fig_generic}(c)) where the user browses and chooses another link.
%The user continues to {\em Website J} (See Figure~\ref{fig_generic}(d)), and at this point, the user wants to go back to where she started and wonders where she was when she originally started browsing.
%The user can use the back option and gauge her location, but there is no context to this method.
%Our system aids in minimizing the frequency of this scenario and is intended to provide a mapping of the location within the cyberspace which is interesting to the user.

%\begin{figure}[bh]
%\centering
%\includegraphics[width=2in]{genericLost}
%\caption{``Lost in cyberspace'' scenario}
%\label{fig_generic}
%\end{figure}

%Another ``lost in cyberspace'' scenario can occur when using the back and forward functionality of a Web browser.  
%The scenario is the following: a user starts at {\em Website A},  then proceeds to {\em Website B}.
%After finding no desired links, the user clicks back to return to {\em Website A}.
%Now, the user chooses {\em Website C}, and after failing to discover interesting information, returns back to {\em Website A}.
%With the back and forward functionality, the user can traverse backwards and forwards in their browsing history, but in this scenario after going to {\em Website C},
%{\em Website B} escapes the forward traversal Web browsing functionality, nor does it appear in the drop-down list that contains pages recently visited.

%Also, the need for our solution exists in creating a more efficient method of Web browsing.
%Better efficiency is established by the creation of {\em ``browsing into the future.''}
%Our solution exposes future Web pages ({\em browsing into the future}), deriving from a starting Web page, without the user having to navigate through the Web pages.

%In addition, Google is extremely successful at guiding the regular Internet user to a desired Website.  Usually this Website is unfamiliar to the regular Internet user, 
%hence the reason why a search engine was needed.
%Once the regular Internet user is at the Website, Google served its purpose, but the regular Internet user could still encounter the "lost in cyberspace" problem.
%Therefore, our solution could extend the strengths of Google (taking the user to a specific area of the WWW) by helping the regular Internet user navigate through different Web pages deriving from the Website and find information easier, and by providing a Web visualization that can answer the questions stemming from the "lost in cyberspace" problem. 
%-----------------------
%Our solution also compliments Google searches:
%a regular Internet user can easily find the Website of his/her interest.  After the search, the user settles on a Website.  Moreover, Google is greatly successful at bringing a regular Internet user to a desired Website, but Google does not provide a clear solution to efficiently navigate within a Website.  In addition to Google's technology, our solution aids the user, 
%once at a desired Website, the user has a clearer understanding of the Website and is able to locate desired information and other related information more efficiently. 
...
%The parameters can be accessed from the Options menu which is located at the top of the browser.
%The corresponding pop-up window is shown in Figure~\ref{fig_paramOpts}.
...
\begin{figure}[t]
\centering
\includegraphics[width=6.25in]{new_screenshot2}
...
%The browser is located in the left part of Figure~\ref{fig_systemshot2}.
...
%In addition, our browser includes a favorites list with capability to add and delete to this list.  
Forward and back buttons are included to allow for traversal through already viewed pages.  
%We discuss the implementation details of our Web browser in Section~\ref{se:implementation}.
...
\begin{figure}[t]
\centering
\includegraphics[width=2.25in]{paramWindow3}
...
%Since our visualization can handle large sized trees, the user can limit the amount of data represented by adjusting the {\em maximum children} parameter to preference. 
...
%Duplicate links are eliminated, because, if included, less information is presented in the visualization, especially in the case of framed Websites. 
%Furthermore, the crawler is halted at a depth denoted by the {\em max depth} parameter.
%The {\em minimum radius} parameter controls the size of the smallest nodes rendered in the visualization.

\begin{figure}[t]
\centering
\includegraphics[width=3.5in]{newState2}
...
%The main problem needed to be resolved was the slow answering servers.  Some servers are slower to respond to the Web crawler.
%Other servers do not respond at all. 
%Being a real-time system, the visualization needs the information from the crawler without delay in order to be generated.
%For this reason, we set up time limits in which a server has to respond.
%This time limit is decided by the user through the combination of {\em response time} and
%{\em disconnect round trip} parameters.
...
%Next, the drawing algorithm will simply continue, assuming that the page that could not be retrieved is not active.  
...
%The interface of {\em WebGPS} has been designed using free floating windows, therefore the user can move each window in a desired location, based on free space available, and minimize, maximize, or close windows.

%In the left window we provide functionality for the user to control the visualization in the middle window.  
%The location and focus of the visualization can be manipulated up, down, left, and right, by using the arrow buttons.  
%Also, the user can zoom in and out by using the buttons labeled '+' and '-'.

%Statistics that describe the system are located in the bottom right window of Figure~\ref{fig_systemshot2}.
...
%The window located on the right of the bottom section of our system is used to provide important information to the user. 
%It is split into three smaller windows based on different categories of information being displayed.
%The top window is used to display the Web address of a node in the visualization over which the mouse is hovering.
%When the mouse is hovering over a node in the visualization, the node changes its color to gray and its corresponding Web address appears in the window.
%This communicates to the user to which Web page will the browser change if the hovered-node is clicked.
%The middle window is used to provide messages to the user, such as errors and instructions.
%Lastly, the bottom window stores a list of all the Web addresses that appear in the visualization.

%The first field of the control parameters is {\em max threads}.  
%Our crawler uses multi-threading, which allows it to connect to more than one server at a time, thus increasing its efficiency in gathering Hyper-Text Markup Language (HTML) source code for parsing.  
%The second field of the control parameters, is {\em response time}, measured in milliseconds (ms).
%The {\em response time} is the amount of time we allow for a thread to establish connection with a server.
%Internet users identify access speed as one of the most common problems of Web browsing~\cite{pr-98}.
%The usage of {\em response time} in our real-time system reduces the negative effect of slow access Web pages by skipping those which do not respond hastily. 
%The next field of the control parameters is the number of {\em disconnect round trips} allowed.  To deal with non-responsive servers, we impose a limit to the number of aborts when trying to connect.  
%For example, with a {\em response time} of $10,000$ ms (ten seconds), a thread attempts to establish connection with a server; 
%upon ten seconds of idle and no connection, the thread retries.  
%With a maximum {\em disconnect round trip} of three, the thread attempts to connect three times and upon the third failure, the server is aborted and the thread will 
%try to establish connection with the next server.
%The last field of the control parameters is {\em child limit}.
%In the structure of a tree, nodes that proceed after another node are called children of that node.
%In our application, a Web page can be viewed as a parent node with its children being represented as Web pages that can be reached from links within that parent Web page.
%The {\em child limit} parameter is used by the crawler to extract a specific number of links per one Web page.
%This allows the user to control the amount of information contained in the visualization.
%A {\em child limit} of six means the crawler will try to connect to a maximum number of six links for each Web page.
%This will create a visualization that has no more than six successors for each node in the tree.

%The first field of the aesthetic parameters is {\em update time}.
%The {\em update time} specifies at what time rate, in milliseconds, to refresh the visualization.
%As the crawler proceeds with traversing through the WWW, more and more links are added to the visualization.
%The data is made available to the user in the visualization with refresh based on {\em update time}.
%The other field of the aesthetic parameters is {\em minimum radius}.
%The {\em minimum radius} allows the user to set the radius of the smallest ring in the visualization.
%This helps the user control the amount and the granularity of the information being displayed at one time in the visualization.

%Complex information spaces such as the WWW are typically polluted with erroneous, redundant, and low
%quality information units which obscure the information units of high value~\cite{a-97}.
%Users of our system could attempt to avoid pollution by seeking information of high value and avoid links which would distract them.
%The user could minimize the harms of Web pollution via the bias parameter.
%The bias parameter includes the {\em page type ban} field alone.
%The field for {\em page type ban} is implemented as a drop-down list where the user can add or delete entries.
%The user can add entries to the list by entering text into the {\em page type ban} textbox which is located directly under the drop down list. 
%({\em {\bf Note:} Textbox not shown in Figure~\ref{fig_paramOpts} since entire drop down list is present in the screen shot.})
%Similarly, the user can delete from the page type ban list.
%During the crawling process, all links from the page are extracted and processed.
%While processing, the crawler neglects the type of links which exist in the page type ban list.
%For example, with the {\em type ban field} containing {\em .pdf, .txt, .css, .ico, mailto, .jpg, .gif}, 
%the links which have these specific filename extensions will be omitted during the Web crawling and retrieval state.
%It is sometimes necessary to omit certain Web pages because only links to pictures or text documents, which in turn would create a ``dead-end'' since they would have no HTML source to parse and could not extract more links.  
...
%A map (visualization) reduces the user's cognitive load because it abates the load on human long term and
%working memory, summarizing the information about the structure and organization that would otherwise have to be remembered~\cite{a-97, w-91}.
%When browsing the WWW, a user's performance is significantly improved if an overview map of the structure of a particular area of interest
%of the WWW is visually represented.
...
%The visualization of the area which the user is browsing is represented as a tree and located in the top right side of Figure~\ref{fig_systemshot2}.
...
%\begin{tabular}{l}
\hspace{0.75in}
{\includegraphics[width=1.2in]{straight-line}}
{\includegraphics[width=1.5in]{polyline}}
\hspace{0.5cm}
{\includegraphics[width=0.90in]{non-planar}}\\
}
\hspace{4.75cm} (a)
\hspace{3.5cm} (b)
\hspace{2.6cm} (c)
...
\begin{figure}[htb]
\centering
\includegraphics[width=4.1in]{enclosing_disc3}
...
%\begin{figure}[t]
%\centering
%\includegraphics[width=1.4in]{ringsGenEx}
...
\begin{figure}[t]
\centering
\includegraphics[width=2.5in]{geoRings2}
...
\begin{figure}[t]
\centering
\includegraphics[width=5.5in]{new_animation_states3}
...
%\begin{figure}[t]
%\centering
%\includegraphics[width=3.4in]{newAnimation}
...
\begin{figure}[t]
\centering
\includegraphics[width=2.0in]{label_slot2}
...
%One possible drawback is the amount of time {\em WebGPS} crawls the WWW depends on the user's speed in browsing.
...
\begin{figure}[t]
\centering
\includegraphics[width=6.25in]{webgps_3levels}
...
\begin{figure}[t]
\centering
\includegraphics[width=6.25in]{webgps_4levelsPS}
...
\begin{figure}[t]
\centering
\includegraphics[width=5.0in]{colored_imagesBETTER}
...
\begin{figure}[tb]
\centering
\includegraphics[width=5.0in]{dummy_nodes}
...
\begin{figure}[tb]
\centering
\includegraphics[width=5.0in]{select_2levelsBETTER}
